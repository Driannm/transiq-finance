...
import { AlertCircle, CheckCircle2, Loader2, Eye, EyeOff, Mail, Lock, User } from "lucide-react";
    ...
    <div className="mt-1.5 space-y-1.5">
      <div className="flex items-center gap-1">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className={["h-[3px] flex-1 rounded-full transition-colors duration-300", i < score ? colors[score - 1] : "bg-gray-200"].join(" ")}
          />
        ))}
        {score > 0 && (
      ...
      <div className="flex gap-3">
        {strengthChecks.map((c) => (
          <span key={c.label} className={["flex items-center gap-1 text-[11px]", c.test(password) ? "text-green-600" : "text-gray-400"].join(" ")}>
      ...
      <div className="w-full max-w-[900px] bg-white rounded-[24px] overflow-hidden shadow-2xl flex min-h-[560px]">
        ...
        <div className="flex-1 flex flex-col p-10 md:p-12 overflow-y-auto">
          ...
          <div className="mb-7">
            ...
            <p className="text-sm text-gray-400">Daftar dan mulai kelola keuangan keluarga</p>
          </div>
          ...
          {/* Error */}
          {serverError && (
            <div role="alert" className="flex items-start gap-2 rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-[13px] text-red-600 mb-5">
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              {errors.name && (
                <p role="alert" className="text-[12px] text-red-500 pl-1 flex items-center gap-1">
                  <AlertCircle size={11} className="shrink-0" />{errors.name.message}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              {errors.confirmPassword && (
                <p role="alert" className="text-[12px] text-red-500 pl-1 flex items-center gap-1">
                  <AlertCircle size={11} className="shrink-0" />{errors.confirmPassword.message}
                </p>
              )}
            </div>
