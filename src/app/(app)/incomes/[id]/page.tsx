...
const safeDate = (iso: string | null | undefined, fmt: string) => {
  if (!iso) return "—";
...
function DetailCard({
  ...
  return (
    <div
      className={`bg-white dark:bg-neutral-900 rounded-3xl border border-gray-100/80 dark:border-neutral-800/80 shadow-sm overflow-hidden ${className}`}
    >
      {children}
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-3.5 px-4 py-3.5 min-h-[56px]">
      ...
      <div className="flex-1 min-w-0 flex items-center justify-between gap-3">
        ...
        <p className="text-[14px] font-semibold text-gray-900 dark:text-gray-100 truncate max-w-[180px] text-right">
          {value}
        </p>
      </div>
    </div>
...
export default function IncomeDetailPage() {
  ...
  const handleDelete = useCallback(() => {
    openConfirm({
      title: "Hapus Income?",
      description: "Income akan dihapus permanen. Saldo kartu akan dikurangi.",
      confirmLabel: "Hapus",
      variant: "danger",
      ...
      <div className="h-screen flex flex-col overflow-hidden bg-neutral-100 dark:bg-neutral-950 font-sans">
        ...
        <div className="flex-1 overflow-y-auto animate-pulse px-4 pt-2 space-y-3">
          ...
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100/80 dark:border-neutral-800/80 h-14"
            />
          ))}
        </div>
      </div>
  ...
  const cardColors =
    CARD_COLORS[income.transaction.card.type] ?? CARD_COLORS.BANK;
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-neutral-100 dark:bg-neutral-950 font-sans">
        ...
        actions={[
          {
            ...
            onPress: () => router.push(`/incomes/${id}/edit`),
            label: "Edit",
          },
        ]}
      />
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        <div className="px-4 pt-2 pb-8 space-y-3">
          ...
          >
            <DetailCard className="px-6 pt-5 pb-6">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[40px] font-bold tracking-tight tabular-nums leading-none text-emerald-600 dark:text-emerald-400">
                  {fmtNum(income.transaction.amount)}
                </span>
              </div>
              <p className="mt-2 text-[13px] text-gray-400 tabular-nums">
                ...
                <span className="text-gray-300 dark:text-gray-600">
                  Dibuat {safeDate(income.transaction.createdAt, "HH:mm")}
                </span>
              </p>
            </DetailCard>
          </motion.div>
          ...
          >
            <DetailCard>
              <DetailRow
                ...
                label="Kartu"
                value={income.transaction.card.name}
              />
              <Divider />
              <DetailRow
                ...
                label="Tanggal"
                value={safeDate(income.transaction.date, "dd MMM yyyy")}
              />
              {income.source && (
                <>
                  <Divider />
          ...
          >
            <p className="text-center text-[11px] text-gray-300 dark:text-gray-700 font-mono">
      ...
      <div className="flex-shrink-0 px-4 pt-3 pb-[max(1.25rem,env(safe-area-inset-bottom))] flex gap-3 bg-gradient-to-t from-neutral-100 dark:from-neutral-950 via-neutral-100/95 dark:via-neutral-950/95 to-transparent">
        ...
        <motion.button
          type="button"
          onClick={() => router.push(`/incomes/${id}/edit`)}
          whileTap={{ scale: 0.97 }}
