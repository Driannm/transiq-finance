...
export default function EditDebtPage({
    ...
    async function fetchDebt() {
      try {
        ...
        const debt: DebtData = {
          id: data.debt.id,
          name: data.debt.name,
          creditor: data.debt.creditor,
          category: data.debt.category || "personal",
          dueDate: data.debt.dueDate,
          totalAmount: data.debt.totalAmount,
        };
        setName(debt.name);
        setCreditor(debt.creditor);
        setCategory(debt.category);
        setDueDate(debt.dueDate ?? "");
        setAmountRaw(formatIDRInput(String(debt.totalAmount)));
      } catch {
        router.push("/debts");
      } finally {
        setLoading(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Gagal memperbarui utang");
      toast.show({ title: "Utang berhasil diperbarui", variant: "success" });
